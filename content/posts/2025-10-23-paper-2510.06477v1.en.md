...
* Definition: an intervention at the layer ℓ where the massive activation appears that removes **only the MLP contribution entering the BOS token**, setting $x ^ {(\ell+1)} * {\text{BOS}} \leftarrow x ^ {(\ell)} * {\text{BOS}} + \text{Attn} ^ {(\ell)} (x_{\text{BOS}})$. **Attention and the MLPs of the other tokens are kept** (source: §3.3) .
