...
* The **core procedure** is the pipeline "**kernel-grain scheduling (time/space) → VM-based memory coexistence (semantics-aware·swapping·KV sharing) → guaranteed/preemptible dual coordination (utility curves) → isolation/deployment integration (K8s DRA)<strong>". This combination is justified with numbers and mechanisms as the practical path for handling the </strong>dynamic memory/compute volatility of the LLM era**. (Source: §3.1–§3.5)
