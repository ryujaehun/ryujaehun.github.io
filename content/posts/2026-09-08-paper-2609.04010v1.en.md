...
**Table 2. Acceptance lengths ($\tau$), throughput (1K/8K test), peak memory usage, and additional parameter counts for Uno$_\text{Qwen}$, EAGLE-3, and DFlash at sampling $\text{temp}=1$. We report the $\tau$ values that maximize system and per-request throughput.**
...
<strong>Table 8. TPFs for the SFT checkpoint and its RL-post-trained counterpart, evaluated using the diffusion weights trained for the SFT checkpoint. </strong>Even after extensive RL post-training, the SFT diffusion adapters retained their speedup<strong>, with only a $6\%$ reduction in TPFs.</strong>
