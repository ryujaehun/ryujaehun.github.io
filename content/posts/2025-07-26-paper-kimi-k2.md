...
* **로짓 폭주 방지 → 학습 안정성**
  Muon류 옵티마이저가 초대형 배치·장훈련에서 보이던 *loss spike*를 <strong>제로(0)</strong>로 만들었다.
* **토큰 효율 극대화**
...
* <strong>공유 (latent) 키 `k_R`</strong>와 <strong>헤드 전용 컴포넌트(`q_C`, `k_C`, `q_R`)</strong>를 분리하여 **장기 컨텍스트**를 효율적으로 재사용한다.
...
* MLA는 **RoPE(Rotary Position Embedding)** 변형을 사용한다.
  ...
  * <strong>헤드 전용 회전(`q_R`)</strong>과 <strong>공유 회전(`k_R`)</strong>으로 분리하여, 각 헤드가 위치 정보에 대해 독립적인 표현력을 가진다.
