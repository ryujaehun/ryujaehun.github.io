...
이 예제에서 `data`는 소유권이 전달되지 않기 때문에 `std::unique_ptr`일 필요가 없습니다. 데이터는 스택에서 `Data` 객체로 선언된 경우와 동일한 시점에서 생성되고 파괴됩니다. 따라서 [팁 #123](/posts/abseil-tip-123-absloptional%EA%B3%BC-stdunique_ptr/)에서 논의된 것처럼, 더 나은 선택은 다음과 같습니다:
...
지연 초기화가 정말로 중요하고 피할 수 없다면, `emplace()` 메서드를 사용하는 `std::optional`을 고려하세요. [팁 #123](/posts/abseil-tip-123-absloptional%EA%B3%BC-stdunique_ptr/)에서 지연 초기화에 대해 더 자세히 다룹니다.
