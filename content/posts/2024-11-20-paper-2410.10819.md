...
- 각 Layer \(i\)와 Head \(j\)에 대해 <strong>Gate Value \(\alpha_{i,j}\)</strong>를 초기화.
...
#### **(2) 실제 데이터셋**
- DuoAttention의 일반화 성능 평가를 위해 **LongBench** 또는 <strong>Needle-in-a-Haystack (NIAH)</strong>와 같은 장문 맥락 처리 데이터셋을 활용합니다.
- 데이터셋 예시:
...
##### **(1) 입력 토큰화**
- 입력 문장을 <strong>토큰(token)</strong>으로 변환:
...
##### **(2) Streaming Head**
- **최근 정보**와 <strong>주의 집중 토큰(Attention Sink)</strong>만 참조.
