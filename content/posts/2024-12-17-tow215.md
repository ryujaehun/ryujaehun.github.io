...
Abseil은 사용자 정의 타입을 문자열로 포맷할 수 있는 새로운 경량 메커니즘인 <strong>`AbslStringify()`</strong>를 지원합니다. `AbslStringify()`를 확장한 사용자 정의 타입은 `absl::StrFormat`, `absl::StrCat`, 그리고 `absl::Substitute`와 함께 바로 사용할 수 있습니다.
