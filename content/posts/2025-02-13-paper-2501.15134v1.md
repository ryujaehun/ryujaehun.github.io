...
1. 주요 강점
	...
	•	대규모 실사용 검증:
	•	ByteDance 내 <strong>12,000+ 주간 활성 사용자(WAU)</strong>와 <strong>210,000+ 주간 페이지 조회(WPV)</strong>를 기록하며, 실제 기업 환경에서 성공적으로 운영됨.
...
① Context Preparation (문맥 준비)
	•	<strong>코드 변경 사항(diff)</strong>을 분석하여 적절한 코드 컨텍스트를 설정.
	•	<strong>트리 기반 분석(tree-sitter 활용)</strong>으로 함수 범위를 파악하고 적절한 코드 스냅샷을 유지.
...
➡ 결론:
	...
	•	<strong>개발자들이 실제 반영하는 코드 리뷰 비율(Outdated Rate)</strong>도 26.7%로 기존보다 우수한 실용성을 보여줌.
...
➡ BitsAI-CR이 특출난 이유:
	...
	•	BitsAI-CR은 <strong>“개발자가 실제로 반영하는 리뷰 비율”</strong>을 분석하여 실질적인 유용성을 평가함.
...
① RuleChecker (초기 코드 리뷰)
	...
	•	LLM이 <strong>환각(Hallucination)</strong>을 일으켜 잘못된 리뷰 코멘트를 생성함.
...
🟢 뛰어난 이유
	...
	•	단순히 “LLM이 리뷰를 잘 생성하느냐”가 아니라, <strong>“개발자가 실제로 반영하는 리뷰인가?”</strong>를 평가한 점이 인상적.
...
LLM 기반 코드 리뷰 시스템을 운영하기 위해 필요한 <strong>컴퓨팅 자원(CPU, GPU, 메모리, 저장소)</strong>을 분석해보자.
...
🔹 Step 2: RuleChecker (리뷰 규칙 적용)
	•	목표: 사전에 정의된 <strong>코드 리뷰 규칙(219개)</strong>을 LLM에 적용하여 문제를 탐지.
...
🔬 연구 방향
	...
	•	AI가 <strong>“이 코드가 왜 변경되었는지”</strong>를 분석하여 리뷰 생성.
