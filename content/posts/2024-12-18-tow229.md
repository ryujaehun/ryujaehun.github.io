...
모호성 오류가 발생하는 상황에서, 명시적인 **클래스 계층**을 사용하여 원하는 방식으로 "최적"을 정의할 수 있습니다. 이를 <strong>순위 기반 오버로드(Ranked Overloads)</strong>라고 합니다. 이 기법은 클래스 계층을 활용하여 우선 순위를 지정하고, 컴파일러가 가장 높은 우선 순위의 메서드를 선택하도록 만듭니다.  
