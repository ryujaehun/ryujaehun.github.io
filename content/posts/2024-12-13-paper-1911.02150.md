...
1. **독창적인 접근법: Multi-query Attention**  
   이 논문은 Transformer 모델의 효율성을 높이기 위해 기존 Multi-head Attention을 Multi-query Attention으로 대체하는 혁신적인 방식을 제안합니다. Multi-query Attention에서는 모든 attention head가 동일한 key-value 세트를 공유합니다. 이를 통해 메모리 대역폭 요구사항이 크게 감소하며, 특히 <strong>증분 디코딩(incremental decoding)</strong>에서 성능을 극적으로 향상시킵니다.
