...
- ML 모델은 <strong>Job의 중요도를 예측(저장소 비용 절감 기여도 측정)</strong>하고, 이 중요도를 기반으로 저장소 계층에서 SSD 배치를 수행한다.
...
- SSD 가용 공간이 <strong>1%</strong>일 때 → **Adaptive Ranking이 기존 Heuristic 대비 4.38배 향상**
- SSD 가용 공간이 <strong>20%</strong>일 때 → **Adaptive Ranking이 기존 Heuristic 대비 1.77배 향상**
...
# **1. 입력 데이터 상세 분석**
논문에서 사용하는 <strong>입력 데이터(Feature Set)</strong>는 Job의 메타데이터, 실행 패턴, 과거 실행 이력 등을 포함한다. 이를 통해 ML 모델이 Job의 중요도를 예측하고 저장소 배치를 최적화한다.
