...
- **저변동성 포트폴리오**: 1달러가 `$59.55`로 성장 (실질 수익률 `$10.12`).
- **고변동성 포트폴리오**: 1달러가 `$0.58`로 감소 (실질 손실).
...
논문에서 제시한 핵심 알고리즘은 <strong>저위험 이상(low-risk anomaly)</strong>을 설명하기 위해 사용된 포트폴리오 분류 및 성과 분석 방법입니다. 이 과정은 아래와 같이 요약할 수 있습니다:
