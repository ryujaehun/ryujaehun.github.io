...
또한, 객체가 안전하게 읽을 수 있는 값으로 초기화되었는지 여부는 명시적인 <strong>초기화자(initializer)</strong>를 사용했는지에 따라 달라집니다. 초기화자는 `()`, `{}`, 또는 `= {}` 형태로 객체 이름 뒤에 나옵니다.

이 규칙은 직관적이지 않으므로, 객체가 초기화되었는지 보장하려면 초기화자를 제공하는 것이 가장 쉬운 방법입니다. 이를 <strong>값 초기화(value-initialization)</strong>라고 하며, 컴파일러가 스칼라 및 집계 타입에 대해 수행하는 <strong>기본 초기화(default-initialization)</strong>와는 다릅니다.
...
위 코드에서 `= {}`는 `value_foo`의 값 초기화를 트리거하며, 이는 `Foo`의 기본 생성자를 호출합니다. 생성자 초기화 리스트가 `v`를 값 초기화하므로, `v`는 안전하게 읽을 수 있습니다. `v`는 클래스 타입이 아니므로 이는 <strong>제로 초기화(zero-initialization)</strong>라는 값 초기화의 특별한 경우로, `value_foo.v`는 `0`이 됩니다.
...
- [Tip #61](/posts/abseil-tip-61-%EA%B8%B0%EB%B3%B8-%EB%A9%A4%EB%B2%84-%EC%B4%88%EA%B8%B0%ED%99%94-default-member-initializers/): Default Member Initializers
- [Tip #88](/posts/abseil-tip-88-%EC%B4%88%EA%B8%B0%ED%99%94-%EB%B0%A9%EB%B2%95-%EA%B7%B8%EB%A6%AC%EA%B3%A0/): Initialization: `=`, `()`, and `{}`  
- [Tip #131](/posts/abseil-tip-131-special-%EB%A9%A4%EB%B2%84-%ED%95%A8%EC%88%98%EC%99%80-default/): Special member functions and `=default`  
