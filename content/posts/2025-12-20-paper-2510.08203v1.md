...
저자들은 SlimPajama-627B에서 **1B tokens**를 샘플링해 빈도 통계를 만들고, “빈도 상위 토큰을 누적해 전체 token occurrences의<strong>40%</strong>를 덮을 때까지”를 function token으로 두며, 그 결과**122 tokens** 가 function token으로 라벨링된다고 제시한다. (근거: §2.2)
...
LLaMA-3.1-8B 아키텍처로 scratch 학습을 수행하며, <strong>1.5B params(2 layers)</strong>및**8B params(32 layers)** 모델을 학습해 비교한다. (근거: §4.1)
...
$$
z=\text{JumpReLU}_\theta(W_{\text{enc}}x+b_{\text{enc}})
$$
(근거: Appx C, Eq.(7))
...
* 저자들은 Gemma2-9B-it에서 해석 가능한 feature를 고르고(식별 방법은 Appendix에 기술), 예시로 **feature 15261 = ‘Speak Chinese’**,<strong> feature 9591 = ‘Russia’</strong>, <strong> feature 13751 = ‘UK’</strong> 를 사용한다. (근거: §3.2, Appx A)
...
$$
v^{S_k}_l=\alpha\cdot W_{\text{dec}}\sum_{i\in S_k} e_i
$$
(근거: Appx A, Eq.(5))
...
* 스케일링 비교로, 1.5B→8B에서 group loss(CE loss, unitless)가 content→function은 **1.90→1.64 (Δ=0.26)**, function→function은<strong> 2.12→1.87 (Δ=0.25)</strong>인 반면, function→content는<strong> 4.88→4.27 (Δ=0.61)</strong>, content→content는 <strong> 3.69→3.08 (Δ=0.61)</strong> 로 더 크게 감소한다고 보고한다. (근거: §4.3)
...
| pre-training에서 consolidation 동학   | LLaMA-3.1-8B arch 기반 scratch 학습: **1.5B Params (2 layers)**,<strong> 8B Params (32 layers)</strong>(근거: §4.1) | SlimPajama-627B<strong> 627B tokens / 1 epoch </strong> (근거: §4.1)                       | Fig.8, Fig.9        |
...
* <strong> 핵심 포인트(저자 주장)</strong> : token degree 분포가 log-log에서 heavy-tail이며, “작은 function token 집합이 대부분 feature를 활성화”한다는 구조를 Fig.5로 제시한다. (근거: Fig.5, §3.1)
...
* Gemma2-9B-it에서 interpretable feature 3개를 선정해 추적하며, 예로 **feature 15261 = ‘Speak Chinese’**,<strong> feature 9591 = ‘Russia’</strong>, <strong> feature 13751 = ‘UK’</strong> 를 사용한다. (근거: §3.2, Fig.6, Appx A)
* “중국어로 러시아 수도/영국 수도” 두 프롬프트에서, content token(예: ‘Chinese’, ‘Russia’, ‘UK’)이 해당 feature를 처음 점화한 뒤, `:`, `the`, `\n` 같은 function token들이 그 activation을 <strong> 전달·재생성(conduit)</strong> 한다는 패턴을 Fig.6으로 제시한다. (근거: §3.2, Fig.6)
...
* 1.5B pre-training에서 training step ~130,000에서 group loss가 **function→content 4.88** ,**content→content 3.69**, **all-tokens 3.31** ,**function→function 2.12**, <strong> content→function 1.90</strong> 로 표시된다(CE loss, unitless). (근거: Fig.9a)
...
| ------------------------- | ----------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------- | -------------------------------- |
| **강한 가정/정의**| function token을 “토큰 빈도”로 정의(상위 빈도 토큰을 누적해 말뭉치 발생의<strong>40%</strong>를 덮을 때까지 추가 →**122 tokens** )하므로, 정의가 코퍼스/언어/토크나이저에 민감할 수 있다 | “function”이 언어학적 기능어와 1:1로 대응하지 않을 수 있고, 다른 말뭉치에서는 40% 커버를 채우는 토큰 집합이 달라져 현상이 약화/변형될 수 있다       | (근거: §3.1).                      |
...
3. 스케일업(더 많은 데이터/컴퓨트/모델 크기)이 loss를 낮추고 learned feature 수를 늘리며, token–feature degree 분포가 <strong>power law(=scale-free)</strong>를 보인다는 점은 관찰되지만,**feature formation 동학과 scale-free의 원인** 은 불명확하다. (근거: §7)
