...
논문 <strong>"Gemma 3 Technical Report"</strong>의 핵심 알고리즘은 **Local-Global Interleaved Attention 구조 + RoPE 재조정 방식**으로, 초장문 컨텍스트 지원(최대 128K tokens)과 **KV cache memory 절감**을 동시에 달성한 것입니다. 여기에 멀티모달 처리를 위한 **SigLIP 기반 시각 토큰 삽입 방식**이 결합됩니다.
