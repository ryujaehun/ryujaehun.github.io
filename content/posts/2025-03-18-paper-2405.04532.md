...
- **QoQ(W4A8KV4) 알고리즘 제안:** 
  - <strong>4비트 가중치(W4), 8비트 활성화(A8), 4비트 KV 캐시(KV4)</strong>를 적용하여 정량화에 따른 정확도 손실을 최소화.
...
#### **1. QoQ 알고리즘 개요**
QoQ(Quattuor-Octō-Quattuor)는 <strong>4비트 가중치(W4), 8비트 활성화(A8), 4비트 KV 캐시(KV4)</strong>를 활용하는 정량화 방법으로, 기존 INT4 기반의 정량화 기법이 가지는 **GPU 성능 저하 문제를 해결**하고 **높은 처리량을 유지**할 수 있도록 설계됨.
...
<strong>기존 방법(W4A4)</strong>는 가중치를 바로 4비트로 변환하여 정확도가 낮고 CUDA 코어에서 많은 연산을 필요로 함.  
...
4. GPU 또는 TPU에서 <strong>mixed precision training (FP16 / BF16)</strong>으로 학습 가속.
...
### **1. QoQ 기반 모델의 입력 데이터**
#### **(1) 입력 데이터 형식**
QoQ를 적용하는 모델은 <strong>대형 언어 모델(LLM)</strong>이므로, 입력 데이터는 **토큰(token) 단위**로 변환됨.
...
##### **✅ 입력 데이터 포맷**
LLM 추론 과정에서는 입력을 **배치(batch), 시퀀스 길이(sequence length), 임베딩 차원(embedding dimension)** 형태의 <strong>텐서(Tensor)</strong>로 변환.
...
- **Quantization-aware Training (QAT) 적용**  
  - 현재 QoQ는 **Post-Training Quantization (PTQ) 방식**이므로, <strong>학습 중 정량화(QAT)</strong>를 적용하면 성능 향상 가능.
