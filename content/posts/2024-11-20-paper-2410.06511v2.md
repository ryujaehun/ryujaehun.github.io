...
논문은 TorchTitan을 활용해 <strong>Llama 3.1 모델 (8B, 70B, 405B 파라미터)</strong>의 훈련 성능을 평가하며, 다음과 같은 성과를 제시합니다:
...
2. **다차원 병렬화의 조합 (3D Parallelism)**:
   ...
   - 특히 **Interleaved 파이프라인 스케줄링**과 <strong>비동기 텐서 병렬화(AsyncTP)</strong>는 기존 방법들이 제공하지 못하는 추가 성능 향상을 가능케 함.
