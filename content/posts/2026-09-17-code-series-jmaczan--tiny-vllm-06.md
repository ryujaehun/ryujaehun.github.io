---
title: "6. KV cache 를 16토큰 블록으로 쪼개기 — block table 로 비연속 블록을 읽는 법"
date: "2026-09-17"
draft: false
...
같은 상수가 [`src/kernels.cu:16-19`](https://github.com/jmaczan/tiny-vllm/blob/e25bf1994efa90bc98b721ba7c527402f86fbeaf/src/kernels.cu#L16-L19) 에 다시 정의되어 있다. 파일 맨 위의 TODO 주석([`src/kernels.cu:6`](https://github.com/jmaczan/tiny-vllm/blob/e25bf1994efa90bc98b721ba7c527402f86fbeaf/src/kernels.cu#L6))이 "main.cpp 과 kernels.cu 사이에 공유하지 말까"라고 이 중복을 지적한다. 커널과 호스트가 같은 값을 쓰지 않으면 주소가 어긋나므로, 이 중복은 "같아야만 하는" 중복이다.

이 레이아웃을 전제로 기록하는 쪽은 5편에서 봤다. 새 토큰의 K 는 `kv_cache + block·BLOCK_BYTES + token_in_block_idx·KV_DIM·sizeof(...)` 위치에, V 는 그 식에 `V_OFFSET` 을 더해 쓴다([`src/main.cpp:866-872`](https://github.com/jmaczan/tiny-vllm/blob/e25bf1994efa90bc98b721ba7c527402f86fbeaf/src/main.cpp#L866-L872)). 이 편이 볼 것은 같은 상수를 읽는 쪽이 어떻게 쓰는지다.
...
이 인덱스는 prefill 의 분산([`src/main.cpp:265`](https://github.com/jmaczan/tiny-vllm/blob/e25bf1994efa90bc98b721ba7c527402f86fbeaf/src/main.cpp#L265)), decode 의 분산([`src/main.cpp:858`](https://github.com/jmaczan/tiny-vllm/blob/e25bf1994efa90bc98b721ba7c527402f86fbeaf/src/main.cpp#L858)), 어텐션 커널([`src/kernels.cu:480`](https://github.com/jmaczan/tiny-vllm/blob/e25bf1994efa90bc98b721ba7c527402f86fbeaf/src/kernels.cu#L480)), 슬롯 반납([`src/main.cpp:1022`](https://github.com/jmaczan/tiny-vllm/blob/e25bf1994efa90bc98b721ba7c527402f86fbeaf/src/main.cpp#L1022)) 네 곳에 같은 모양으로 등장한다. 첫 항이 슬롯, 둘째가 레이어, 셋째가 그 레이어 안의 논리 블록이다. 같은 물리 블록 번호라도 어느 슬롯·어느 레이어의 것이냐에 따라 이 인덱스로 구분된다.
...
런치는 `pagedAttentionKernel<<<dim3(num_active_slots, NUM_Q_HEADS), HEAD_DIM>>>`([`src/kernels.cu:527`](https://github.com/jmaczan/tiny-vllm/blob/e25bf1994efa90bc98b721ba7c527402f86fbeaf/src/kernels.cu#L527))다. 블록 하나가 64 스레드, 곧 워프 2개다. 시그니처의 `num_active_slots` 는 커널 본문에서 참조되지 않는다. 그리드의 x 크기가 곧 블록 수를 정하기 때문이다.
...
| 인자 | 역할 | 근거 |
| --- | --- | --- |
| `kv_cache` | 2GB 를 블록으로 쪼갠 KV 저장소. 물리 블록 주소의 기준 | [`src/kernels.cu:461`](https://github.com/jmaczan/tiny-vllm/blob/e25bf1994efa90bc98b721ba7c527402f86fbeaf/src/kernels.cu#L461), [`src/main.cpp:576`](https://github.com/jmaczan/tiny-vllm/blob/e25bf1994efa90bc98b721ba7c527402f86fbeaf/src/main.cpp#L576) |
| `block_table_gpu` | 논리 블록 → 물리 블록 매핑. slot·레이어·논리 블록 세 항목 인덱스 | [`src/kernels.cu:480`](https://github.com/jmaczan/tiny-vllm/blob/e25bf1994efa90bc98b721ba7c527402f86fbeaf/src/kernels.cu#L480), [`src/main.cpp:878`](https://github.com/jmaczan/tiny-vllm/blob/e25bf1994efa90bc98b721ba7c527402f86fbeaf/src/main.cpp#L878) |
| `gpu_seq_lens` | 활성 슬롯별 현재 길이. 읽을 토큰·블록 수를 정한다 | [`src/kernels.cu:470`](https://github.com/jmaczan/tiny-vllm/blob/e25bf1994efa90bc98b721ba7c527402f86fbeaf/src/kernels.cu#L470), [`src/main.cpp:751-757`](https://github.com/jmaczan/tiny-vllm/blob/e25bf1994efa90bc98b721ba7c527402f86fbeaf/src/main.cpp#L751-L757) |
| `gpu_active_slots` | 이번 스텝에 계산할 실제 슬롯 목록. `active_slot` → `slot` 변환 | [`src/kernels.cu:465`](https://github.com/jmaczan/tiny-vllm/blob/e25bf1994efa90bc98b721ba7c527402f86fbeaf/src/kernels.cu#L465), [`src/main.cpp:750`](https://github.com/jmaczan/tiny-vllm/blob/e25bf1994efa90bc98b721ba7c527402f86fbeaf/src/main.cpp#L750) |
| `q_proj` | 입력 Q. 활성 슬롯 순서로 압축된 `buf_2048_1` | [`src/kernels.cu:469`](https://github.com/jmaczan/tiny-vllm/blob/e25bf1994efa90bc98b721ba7c527402f86fbeaf/src/kernels.cu#L469), [`src/main.cpp:763`](https://github.com/jmaczan/tiny-vllm/blob/e25bf1994efa90bc98b721ba7c527402f86fbeaf/src/main.cpp#L763) |
| `output` | 출력. `q_proj` 와 같은 버퍼에 쓰인다 | [`src/kernels.cu:522`](https://github.com/jmaczan/tiny-vllm/blob/e25bf1994efa90bc98b721ba7c527402f86fbeaf/src/kernels.cu#L522), [`src/main.cpp:878`](https://github.com/jmaczan/tiny-vllm/blob/e25bf1994efa90bc98b721ba7c527402f86fbeaf/src/main.cpp#L878) |

다른 decode 커널과 대비하면 이 조합이 왜 어텐션에만 있는지가 분명하다. `embeddingGatherKernelDecode` 는 `gpu_last_tokens`·임베딩·출력만 받고([`src/kernels.cu:347`](https://github.com/jmaczan/tiny-vllm/blob/e25bf1994efa90bc98b721ba7c527402f86fbeaf/src/kernels.cu#L347)), `ropeKernelDecode` 는 입력·위치·차원만 받으며([`src/kernels.cu:371`](https://github.com/jmaczan/tiny-vllm/blob/e25bf1994efa90bc98b721ba7c527402f86fbeaf/src/kernels.cu#L371)), `softmaxKernelDecode` 는 입력과 길이만 받는다([`src/kernels.cu:408`](https://github.com/jmaczan/tiny-vllm/blob/e25bf1994efa90bc98b721ba7c527402f86fbeaf/src/kernels.cu#L408)). KV 저장소와 그 매핑, 그리고 이번 스텝에 계산할 슬롯과 길이를 한꺼번에 알아야 하는 커널은 어텐션뿐이다.

여기 두 인덱스 공간이 갈린다는 점을 짚자. `gpu_active_slots[active_slot]`([`src/kernels.cu:465`](https://github.com/jmaczan/tiny-vllm/blob/e25bf1994efa90bc98b721ba7c527402f86fbeaf/src/kernels.cu#L465))과 `gpu_seq_lens[active_slot]`([`src/kernels.cu:470`](https://github.com/jmaczan/tiny-vllm/blob/e25bf1994efa90bc98b721ba7c527402f86fbeaf/src/kernels.cu#L470))은 압축된 인덱스 `active_slot` 으로 읽는다. 반면 `block_table_gpu` 는 실제 슬롯 `slot` 으로 인덱스한다([`src/kernels.cu:480`](https://github.com/jmaczan/tiny-vllm/blob/e25bf1994efa90bc98b721ba7c527402f86fbeaf/src/kernels.cu#L480)). q 입력과 출력도 `active_slot` 기준이다([`src/kernels.cu:469`](https://github.com/jmaczan/tiny-vllm/blob/e25bf1994efa90bc98b721ba7c527402f86fbeaf/src/kernels.cu#L469), `522`). block table 은 시퀀스별로 블록을 할당받으니 실제 슬롯 번호로, 계산 중인 데이터는 이번 배치 안의 순서로 인덱스되는 것이다. 활성 슬롯 목록이 그 두 공간을 잇는 다리다.
...
셋째, 블록 안에서 유효한 토큰 수를 `min` 으로 끊는다([`src/kernels.cu:481`](https://github.com/jmaczan/tiny-vllm/blob/e25bf1994efa90bc98b721ba7c527402f86fbeaf/src/kernels.cu#L481)). 마지막 블록은 16개를 못 채울 수 있으므로, 시퀀스 길이에서 지나온 블록 수×16 을 뺀 만큼만 읽는다.
...
두 인덱스가 같으므로 각 스레드는 자기 원소 하나를 읽어 루프를 돈 뒤 같은 자리에 쓴다. 블록 차원이 Q 헤드(`blockIdx.y`)라서 다른 헤드의 구간과도 겹치지 않는다. 호스트 쪽에서도 그대로 보인다. `q_proj` 는 `buf_2048_1` 을 가리키고([`src/main.cpp:763`](https://github.com/jmaczan/tiny-vllm/blob/e25bf1994efa90bc98b721ba7c527402f86fbeaf/src/main.cpp#L763)), `pagedAttention` 호출은 같은 `buf_2048_1` 을 입력이자 출력으로 넘긴다([`src/main.cpp:878`](https://github.com/jmaczan/tiny-vllm/blob/e25bf1994efa90bc98b721ba7c527402f86fbeaf/src/main.cpp#L878)). 그 결과가 곧바로 O 투영의 입력이 된다([`src/main.cpp:892`](https://github.com/jmaczan/tiny-vllm/blob/e25bf1994efa90bc98b721ba7c527402f86fbeaf/src/main.cpp#L892)).
...
`free_blocks` 는 0부터 65535까지로 시작한다([`src/main.cpp:577-578`](https://github.com/jmaczan/tiny-vllm/blob/e25bf1994efa90bc98b721ba7c527402f86fbeaf/src/main.cpp#L577-L578)). prefill 은 논리 블록 위치가 `-1` 일 때만, decode 는 `token_in_block_idx == 0` 일 때만 `pop_back` 으로 새 물리 블록을 꺼낸다([`src/main.cpp:266-272`](https://github.com/jmaczan/tiny-vllm/blob/e25bf1994efa90bc98b721ba7c527402f86fbeaf/src/main.cpp#L266-L272), `859-865`). 슬롯이 종료되면 세 차원 순회로 그 슬롯이 소유한 블록을 찾아, `-1` 이 아닌 것만 `push_back` 하고 다시 `-1` 로 되돌린다. 반납된 블록은 이후 다른 슬롯의 할당이 다시 쓸 수 있다. 슬롯 종료가 어떤 조건에서 일어나는지는 7편이 다룬다.

block table 의 변경은 디바이스 사본과 세 번 통째로 맞춘다. prefill 후([`src/main.cpp:552`](https://github.com/jmaczan/tiny-vllm/blob/e25bf1994efa90bc98b721ba7c527402f86fbeaf/src/main.cpp#L552)), decode 레이어마다([`src/main.cpp:876`](https://github.com/jmaczan/tiny-vllm/blob/e25bf1994efa90bc98b721ba7c527402f86fbeaf/src/main.cpp#L876)), 반납 후([`src/main.cpp:1030`](https://github.com/jmaczan/tiny-vllm/blob/e25bf1994efa90bc98b721ba7c527402f86fbeaf/src/main.cpp#L1030))다. 매번 4096 항목을 H2D 로 복사하는 이 동기화에는 "전체 테이블을 불필요하게 복사하지 않게" 하는 TODO 주석이 달려 있다([`src/main.cpp:551`](https://github.com/jmaczan/tiny-vllm/blob/e25bf1994efa90bc98b721ba7c527402f86fbeaf/src/main.cpp#L551)).
...
빌드도 실행도 하지 않았고, 위 모든 설명은 커밋 `e25bf19` 의 소스를 읽어 얻은 정적 인용이다. 특히 네 지점은 실행 검증 없이 남는다. 첫째, block table 의 세 항목 인덱스 산술(`slot·N_LAYERS·MAX_BLOCKS_PER_SEQ + layer·MAX_BLOCKS_PER_SEQ + block_idx`)의 실행 정합성은 GPU 없이 검증되지 않았다. 4096 항목과 65536 블록은 상수 산술이다. 둘째, `dot_products[2]` 와 스레드 0·32 의 협력은 `HEAD_DIM = 64` 전제이며 그 외 크기에서는 성립하지 않는다. 셋째, in-place 출력의 안전성은 읽기·쓰기 인덱스 동일성이라는 정적 주장이며, 동시성·경합은 실행 검증되지 않았다. 넷째, 레이어마다 block table 4096 항목을 통째로 H2D 복사하는 비용([`src/main.cpp:876`](https://github.com/jmaczan/tiny-vllm/blob/e25bf1994efa90bc98b721ba7c527402f86fbeaf/src/main.cpp#L876), TODO [`src/main.cpp:551`](https://github.com/jmaczan/tiny-vllm/blob/e25bf1994efa90bc98b721ba7c527402f86fbeaf/src/main.cpp#L551))은 측정하지 않았다. 가중치 파일은 gated 모델 `meta-llama/Llama-3.2-1B-Instruct` 의 `model.safetensors` 라, paged attention 을 실제로 돌려 보는 검증은 후속 과제로 남는다.
