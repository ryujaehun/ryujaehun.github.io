...
| ------------------ | ---------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------- |
| **전문가 수**      | 8 \~ 16개 FFN 전문가, Top-1/2 라우팅(GShard, Switch) | ▹ 한 전문가가 *다양한* 토큰을 학습 → **지식 혼합(Hybridity)**<br>▹ 여러 전문가가 *같은* 기초 지식 반복 학습 → **지식 중복(Redundancy)** | **m-way 세분화** → 전문가 수 × m, 활성 전문가 수도 × m → 토큰별 조합 $_64C8 ≈ 4.4 × 10⁹$ 으로 폭증 |
...
> 📌 결과적으로 DeepSeekMoE **16 B**는 **LLaMA-2 7 B**와 맞먹는 성능을 <strong>연산량 0.4×</strong>로 달성하고, **145 B** 모델은 Dense 67 B 수준을 <strong>FLOPs 28.5 %</strong>로 재현한다 .
...
| **② 연산·파라미터 효율 ↗**                                  | **16 B 모델**이 LLaMA-2 7 B와 **동급 성능**을 <strong>FLOPs 0.40×</strong>로 달성; **145 B 모델**이 Dense 67 B와 맞먹는 성능을 <strong>FLOPs 0.285×</strong>로 달성 | “With only 40 % computations, DeepSeekMoE 16 B achieves comparable performance … DeepSeekMoE 145 B matches DeepSeek 67 B with just 28.5 % computations” |           |
...
> 실험적으로 **2 B 모델**이 동일 FLOPs에서 GShard 2 B 대비 **+3 pp** 평균 성능을, **16 B**는 LLaMA-2 7 B와 동급 성능을 <strong>연산량 0.4×</strong>로 달성 .  
...
* **가장 강력한 근거 ✔️** 同 FLOPs · 비슷한 activated parameters 조건에서
  ...
  2. **16 B 모델**이 **LLaMA-2 7 B**를 10개 벤치 중 7개에서 앞서면서도 <strong>연산량 39.6 %</strong>에 불과
...
2. **세밀한 전문가 마스킹 실험에서 민감도 ↑**
   Top-routed expert 하나만 disable 해도 GShard보다 손실 증가폭이 크다 → 파라미터 중복이 적어 <strong>“불안정할 수 있다”</strong>고 시사
...
> **요약** 저자는 <strong>“145 B 모델 완성·공개, 전문가 더 세밀화, Attention 보강”</strong>을 직접 지목했다. 이 리뷰는 여기에 **통신 최적화, 하이브리드 MoE, 안전성·프라이버시 강화**를 더해 **성능-효율-책임성 3박자**를 갖춘 차세대 연구 로드맵을 제안한다.
...
DeepSeekMoE는 <strong>“연산량(FLOPs)·활성 파라미터·토큰/초 TPS”</strong>를 핵심 효율 지표로 삼아 성능을 평가한다. 16 B 모델 기준, **FLOPs는 LLaMA-2 7 B의 39.6 %**, **단일 A100-40 GB 추론 TPS는 ≈ 2.5 × ↑**, 그리고 **GPU 40 GB VRAM 안에 탑재** 가능함을 보여주며, 145 B까지 확장해도 Dense 67 B 대비 **계산 28.5 %** 만으로 동급 성능을 유지한다.
