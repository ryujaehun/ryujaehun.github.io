...
이 엔진에서 `main()` 은 cuBLAS 핸들을 만든 뒤 곧바로 두 가지를 한다(`src/main.cpp:557-569`). `model.safetensors` 파일에서 가중치를 꺼내 GPU 메모리에 올리는 일, 그리고 앞으로 모든 계산이 쓸 버퍼를 미리 잡아 두는 일. 이번 편은 그 둘을 읽는다.
...
이 편이 다루는 건 그중 두 단계, `loadWeights`(`src/main.cpp:79-147`)와 버퍼 할당 구간(`src/main.cpp:617-693`)이다. 같은 초기화 구간에는 RoPE 주파수 테이블(`src/main.cpp:572`)과 요청 큐(`src/main.cpp:583-594`)도 만들지만, 그것은 각각 3편과 7편의 몫이다.
...
함수는 네 단계로 나뉜다. 먼저 `checkGPUStatus`(`src/main.cpp:40-62`)로 디바이스가 있는지 보고, 없으면 오류를 출력하고 `return 1` 한다(`src/main.cpp:44-48`). 그다음 파일을 연다. 경로가 인자가 아니라 문자열 그대로 적혀 있는 것부터 모델 구조가 고정돼 있다는 전제의 첫 징표다.
...
버퍼 설계의 원칙은 1편에서 봤다. `cudaMalloc` 은 비싸니 한 번에 전부 잡고 재사용한다. 할당은 `src/main.cpp:617-693` 한 구간에 모여 있다. 크기는 파일 맨 위의 컴파일 타임 상수(`src/main.cpp:16-31`)가 정한다. 그중 핵심은 `MAX_BUFFER_SIZE` 다.
...
| --- | --- | --- | --- | --- |
| `gpu_input_tokens` | `MAX_PROMPT_LEN × sizeof(int)` | 2 KiB | 프롬프트 토큰 ID 를 GPU 로 올리는 임시 버퍼 | `src/main.cpp:617-618` |
| `input_embeddings` | `MAX_PROMPT_LEN × EMBEDDING_LENGTH × 2` | 2 MiB | 토큰 임베딩 결과 | `src/main.cpp:619-620` |
| `hidden_state` | `MAX_BUFFER_SIZE × EMBEDDING_LENGTH × 2` | 2 MiB | 레이어 입력·출력, 잔차 누적 | `src/main.cpp:622-623` |
...
| `k_proj_temp_buf` | `MAX_PROMPT_LEN × KV_DIM × 2` | 512 KiB | prefill K 투영 임시 버퍼 | `src/main.cpp:635-636` |
| `v_proj_temp_buf` | `MAX_PROMPT_LEN × KV_DIM × 2` | 512 KiB | prefill V 투영 임시 버퍼 | `src/main.cpp:638-639` |
...
| `embed_proj_cpu` | `MAX_BUFFER_SIZE × VOCAB_SIZE` 요소 (호스트) | 125.25 MiB | 로짓 D2H 수신 | `src/main.cpp:681-682` |
...
처음에는 Q 투영이 `buf_2048_1` 에 결과를 쓴다. 그다음 헤드별 어텐션 점수 계산(`src/main.cpp:301-327`)이 이 Q 를 전부 읽는다. 그 뒤에야 343 에서 같은 버퍼가 점수×V 의 출력으로 다시 쓰인다. Q 가 살아 있는 동안 덮어쓰지 않으니 안전하다. 이 안전성은 소스에 나타난 "읽는 순서"라는 정적 사실로만 보장되고, 실행으로 검증된 것은 아니다.
...
377 에서 `o_proj = buf_2048_2` 로 다시 쓰이고, 이어지는 cuBLAS(`src/main.cpp:378-396`)가 O 투영 출력을 기록한다. `residualAdd`(`src/main.cpp:399`)가 그 값을 읽어 `hidden_state` 에 더한 뒤, 같은 버퍼가 470 에서 down 프로젝션 출력으로 다시 쓰인다. O 와 down 의 결과가 살아 있는 시점은 겹치지 않는다.

decode 경로에서도 이 별칭이 이어진다. `pagedAttention` 의 출력이 다시 `buf_2048_1`(`src/main.cpp:878`)에 기록되고, 그것이 곧바로 O 투영의 입력이 된다. 이 in-place 쓰기가 안전한 이유는 6편이 다룬다.

## 초기화 구간에 함께 준비되는 것들

계산 버퍼 16개 외에도 같은 초기화 구간에 세 가지가 만들어진다.

- paged KV cache 할당자(`src/main.cpp:574-581`) — 2GB `kv_cache` 와 `free_blocks`(0..65535), `block_table`(전부 -1)이다. PagedAttention 은 KV cache 를 연속된 큰 덩어리가 아니라 작은 블록(페이지)으로 쪼개 관리하는 기법이다. 1편에서 그 준비를 보았고, 이 구조를 읽고 쓰는 어텐션 커널은 6편이 다룬다. ([Kwon et al., SOSP 2023](https://arxiv.org/pdf/2309.06180))
- RoPE 주파수 테이블(`src/main.cpp:572`) — 3편의 몫이다.
- 요청 큐(`src/main.cpp:583-594`) — 7편의 몫이다.
