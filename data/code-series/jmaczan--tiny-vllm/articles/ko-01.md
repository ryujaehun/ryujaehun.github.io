...
단위를 규정한다(`CMakeLists.txt:49-52`). 즉 제품 바이너리 `tiny-vllm` 은
`src/main.cpp` 와 `src/kernels.cu` 두 파일만 컴파일해 만들어진다.

두 번역 단위의 분담은 소스에서 확인된다. `src/main.cpp` 는 호스트 쪽
진입점과 오케스트레이션을 담는다: `main`(`src/main.cpp:555-556`), 가중치 적재
`loadWeights`(`src/main.cpp:79-147`), 프롬프트를 흘리는
`prefill`(`src/main.cpp:150-553`), 그리고 decode 루프를 포함한 `main`
본문(`src/main.cpp:555-1044`)이다. `src/kernels.cu` 는 디바이스 커널을
담는다. `__global__` 정의가 정확히 11개 있고, prefill 계열 7개
(`src/kernels.cu:32`, `55`, `173`, `224`, `257`, `311`, `331`)와 decode 계열
4개(`src/kernels.cu:347`, `371`, `408`, `461`)로 나뉜다.

헤더는 번역 단위로 세지 않는다. `CMakeLists.txt` 는 include 경로로 `src` 와
`include` 를 추가한다(`CMakeLists.txt:62-63`). `src/main.cpp` 는 그 경로를
통해 세 헤더를 끌어온다: `cuda_to_hip.h`(`src/main.cpp:4`),
`json.hpp`(`src/main.cpp:7`), `kernels.cuh`(`src/main.cpp:8`). 이 중
`json.hpp` 는 제품 소스가 아니라 include 경로로 들어오는 제3자 단일
헤더(`JSON for Modern C++` 3.12.0, MIT, Niels Lohmann)다. 그 버전과 라이선스는
헤더 자체에 박혀 있고(`include/json.hpp:6-7`, `include/json.hpp:68-70`), 제품
코드에서의 사용은 safetensors 헤더 파싱 한 곳뿐이다: 별칭 선언
(`src/main.cpp:10`), JSON 파싱(`src/main.cpp:104`), 텐서 순회
(`src/main.cpp:106`). 이 파일은 분석 대상 소스 바이트의 약 92% 를 차지하지만
...
| 경로 | 제품에서의 지위 | 근거 |
| --- | --- | --- |
| `src/main.cpp` | 제품 번역 단위. 호스트 진입점과 오케스트레이션을 담는다 | `CMakeLists.txt:49-52`, `src/main.cpp:555-1044` |
| `src/kernels.cu` | 제품 번역 단위. `__global__` 커널 11개를 정의한다 | `CMakeLists.txt:49-52`, `src/kernels.cu:32-523` |
| `include/json.hpp` | 제품 번역 단위가 아님. `include` 경로로 들어오는 단일 헤더 의존 | `CMakeLists.txt:62-63`, `src/main.cpp:7` |

표가 답하는 필수 주장은 `product-translation-units` 다. 두 요소를 모두 이
표가 답한다.

- 제품 바이너리는 `src/main.cpp` 와 `src/kernels.cu` 두 번역 단위로만
  만들어진다(`CMakeLists.txt:49-52`).
- `include/json.hpp` 는 제품 코드가 아니라 include 경로로 들어오는 단일
  헤더 의존이다(`CMakeLists.txt:62-63`, `src/main.cpp:7`).

## 같은 소스, 두 백엔드

`CMakeLists.txt` 는 `option(USE_HIP "Build with HIP for AMD GPUs" OFF)` 로
기본값이 꺼진 옵션을 둔다(`CMakeLists.txt:3`). 이 옵션 하나가 언어·툴체인·링크
대상을 통째로 가른다.

- 언어. `USE_HIP` 면 `project(tiny-vllm LANGUAGES CXX HIP)`
  (`CMakeLists.txt:13`), 아니면 `LANGUAGES CXX CUDA` 다(`CMakeLists.txt:15`).
- 컴파일러. HIP 이 아니면 `nvcc` 경로를 지정한다(`CMakeLists.txt:5-8`).
- CUDA 표준·아키텍처. HIP 이 아닐 때만 `CMAKE_CUDA_STANDARD 17` 과
  `CMAKE_CUDA_ARCHITECTURES 120` 을 정한다(`CMakeLists.txt:21-25`).
- 컴파일 플래그. 백엔드별로 Release/DEBUG 플래그를 따로 둔다
  (`CMakeLists.txt:32-38`).
- 의존 패키지. HIP 이면 `hipblas`·`hip`(`CMakeLists.txt:42-44`), 아니면
  `CUDAToolkit` 이다(`CMakeLists.txt:46`).
- 번역 단위 언어. HIP 이면 `main.cpp`·`kernels.cu` 를 HIP 으로 컴파일하고
  `USE_HIP` 매크로를 정의한다(`CMakeLists.txt:54-60`).
- 링크. HIP 이면 `hip::host`·`roc::hipblas`(`CMakeLists.txt:65-69`), 아니면
  `CUDA::cublas`·`CUDA::cudart` 다(`CMakeLists.txt:71-74`).

소스가 두 백엔드를 모두 견디는 이유는 `src/cuda_to_hip.h` 다. 이 헤더는
`#if defined(USE_HIP) || defined(__HIP_PLATFORM_AMD__)` 한 조건으로
갈린다(`src/cuda_to_hip.h:6`). HIP 쪽에서는
`hip_runtime.h`·`hip_bf16.h`·`hipblas.h` 를 끌어오고
(`src/cuda_to_hip.h:8-10`), bfloat16 타입을 `__nv_bfloat16` 에서
`__hip_bfloat16` 으로 매핑하며(`src/cuda_to_hip.h:13-14`), `cudaMalloc`·
`cudaMemcpy` 같은 런타임 이름과 `cublasCreate`·`cublasGemmEx` 같은 BLAS
이름을 매크로로 바꾼다(`src/cuda_to_hip.h:17-31`, `src/cuda_to_hip.h:33-46`).
CUDA 쪽에서는 표준 헤더를 그대로 쓴다(`src/cuda_to_hip.h:52-59`).
`src/main.cpp:4` 와 `src/kernels.cu:1` 이 이 헤더를 include 하는 지점이다.
...
에서는 32비트 `0xffffffff` 로 정의한다(`src/cuda_to_hip.h:48-50`,
`src/cuda_to_hip.h:58-59`). 이 상수를 실제로 소비하는 곳은
`pagedAttentionKernel` 내부의 shuffle 다섯 줄이다
(`src/kernels.cu:489-493`). 그 커널의 동작은 6편이 소유한다.
...
하드코딩된 토큰 ID 열을 표준 입력으로 `./test.sh` 에 넘긴다(`full_test.sh:1`).
그 열은 Llama-3 채팅 템플릿 형태의 토큰 ID 42개다. `test.sh` 는 두 줄로, 먼저
`./build.sh` 를 부르고 곧바로 `./run.sh` 를 부른다(`test.sh:1-2`). `build.sh`
는 `build/` 를 지우고 다시 만들고 `cmake .. -G Ninja && ninja` 로
빌드한다(`build.sh:1-3`). `run.sh` 는 `./build/tiny-vllm` 을
실행한다(`run.sh:1`).
...
C++ 진입점은 `main` 이다(`src/main.cpp:555-556`). 첫 행위는 cuBLAS 핸들
생성이고 실패하면 `return 1` 한다(`src/main.cpp:557-563`). 곧바로 `loadWeights`
로 가중치를 올리고(`src/main.cpp:566-569`), RoPE 주파수 테이블을 초기화한
뒤(`src/main.cpp:572`), paged KV cache 할당자를 세운다(`src/main.cpp:574-581`).
이어 요청 큐(`src/main.cpp:583-594`)와 슬롯·배치 상태(`src/main.cpp:597-610`)를
준비하고, 연산 버퍼를 잡는다(`src/main.cpp:617-693`). 초기 prefill 루프가 빈
슬롯을 큐의 프롬프트로 채우고(`src/main.cpp:695-708`), 그 뒤 `while (true)`
decode 루프가 돈다(`src/main.cpp:720-1039`). 이 루프의 유일한 break 는 큐가
빈 상태에서 활성 슬롯이 모두 사라졌을 때다(`src/main.cpp:738-746`). break
뒤에는 `Ok bye!` 를 출력하고 핸들을 정리한 뒤 `return 0`
한다(`src/main.cpp:1040-1044`).

호스트 함수는 네 개뿐이다. `checkGPUStatus`(`src/main.cpp:40-62`),
`loadWeights`(`src/main.cpp:79-147`), `prefill`(`src/main.cpp:150-553`),
`main`(`src/main.cpp:555-1044`)이다. 모델 forward 전체와 paged KV cache 관리가
...
(`src/main.cpp:555`). 그러나 본문 어디에서도 `argc`·`argv` 를 읽지 않고, 표준
...
네 개를 `queue` 에 push 한다(`src/main.cpp:583-594`). 주석이 각 프롬프트를
"What is 2+2?"(길이 17), "Name a color."(길이 14), "Say hello."(길이 13),
"Capital of France?"(길이 14)로 표시한다(`src/main.cpp:583`,
`src/main.cpp:587`, `src/main.cpp:590`, `src/main.cpp:593`). 경로도 인자가
아니라 문자열로 박혀 있다: `loadWeights` 는 작업 디렉터리의 `model.safetensors`
를 연다(`src/main.cpp:87`).
...
- `CMakeLists.txt:1-75` — 언어·백엔드 분기, 번역 단위, include 경로, 링크.
- `build.sh:1-3`, `run.sh:1`, `test.sh:1-2`, `full_test.sh:1` — 실행 경로.
- `src/cuda_to_hip.h:1-61` — bfloat16·런타임·BLAS shim 과 `WARP_FULL_MASK`.
- `src/kernels.cuh:1-28` — 커널 선언과 bfloat16 별칭.
- `src/main.cpp:1-10` — include 블록.
- `src/main.cpp:40-62`, `79-147`, `150-553`, `555-1044` — 호스트 함수 네 개.
- `src/main.cpp:583-594` — 하드코딩 프롬프트 큐.
- `src/main.cpp:720-746`, `1040-1044` — decode 루프의 break 와 정상 종료.
- `src/kernels.cu:32-523` — `__global__` 커널 11개.
- `include/json.hpp:6-7`, `68-70` — vendored 헤더의 SPDX 라이선스와 버전.
