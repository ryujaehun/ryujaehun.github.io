...
| 경로 | 제품에서의 위치 | 근거 |
| --- | --- | --- |
| `src/main.cpp` | 컴파일 대상. 호스트 쪽 전부 | [`CMakeLists.txt:49-52`](https://github.com/jmaczan/tiny-vllm/blob/e25bf1994efa90bc98b721ba7c527402f86fbeaf/CMakeLists.txt#L49-L52) |
| `src/kernels.cu` | 컴파일 대상. `__global__` 커널 11개 | [`CMakeLists.txt:49-52`](https://github.com/jmaczan/tiny-vllm/blob/e25bf1994efa90bc98b721ba7c527402f86fbeaf/CMakeLists.txt#L49-L52) |
| `include/json.hpp` | 컴파일 대상 아님. include 경로로 들어오는 남의 헤더 | [`CMakeLists.txt:62-63`](https://github.com/jmaczan/tiny-vllm/blob/e25bf1994efa90bc98b721ba7c527402f86fbeaf/CMakeLists.txt#L62-L63), [`src/main.cpp:7`](https://github.com/jmaczan/tiny-vllm/blob/e25bf1994efa90bc98b721ba7c527402f86fbeaf/src/main.cpp#L7) |

## 실제로 돌리는 방법

저장소가 정해 둔 실행 경로는 셸 스크립트 네 개다. `full_test.sh` 가 토큰 ID 열을 표준 입력으로 [`test.sh`](https://github.com/jmaczan/tiny-vllm/blob/e25bf1994efa90bc98b721ba7c527402f86fbeaf/test.sh) 에 넘기고([`full_test.sh:1`](https://github.com/jmaczan/tiny-vllm/blob/e25bf1994efa90bc98b721ba7c527402f86fbeaf/full_test.sh#L1)), `test.sh` 는 `build.sh` 와 `run.sh` 를 차례로 부른다([`test.sh:1-2`](https://github.com/jmaczan/tiny-vllm/blob/e25bf1994efa90bc98b721ba7c527402f86fbeaf/test.sh#L1-L2)). `build.sh` 는 `build/` 를 지우고 `cmake .. -G Ninja && ninja` 로 새로 빌드하고([`build.sh:1-3`](https://github.com/jmaczan/tiny-vllm/blob/e25bf1994efa90bc98b721ba7c527402f86fbeaf/build.sh#L1-L3)), `run.sh` 는 `./build/tiny-vllm` 을 실행한다([`run.sh:1`](https://github.com/jmaczan/tiny-vllm/blob/e25bf1994efa90bc98b721ba7c527402f86fbeaf/run.sh#L1)). 즉 매번 클린 빌드다.

재미있는 건 그 표준 입력이 아무 데도 닿지 않는다는 점이다. `main` 은 `int main(int argc, char *argv[])` 로 선언돼 있지만([`src/main.cpp:555`](https://github.com/jmaczan/tiny-vllm/blob/e25bf1994efa90bc98b721ba7c527402f86fbeaf/src/main.cpp#L555)) 본문에서 `argc`·`argv` 를 읽지 않고, `std::cin` 도 `scanf` 도 없다. `full_test.sh` 가 넘긴 토큰 열은 무시되고, 실제로 돌아가는 건 위에서 본 하드코딩 프롬프트 네 개다. 가중치 경로도 인자가 아니라 작업 디렉터리의 `model.safetensors` 로 박혀 있다([`src/main.cpp:87`](https://github.com/jmaczan/tiny-vllm/blob/e25bf1994efa90bc98b721ba7c527402f86fbeaf/src/main.cpp#L87)).
...
flowchart TD
    ...
    OPT -->|"OFF (기본)"| CUDA["LANGUAGES CXX CUDA<br/>CUDA::cublas · CUDA::cudart"]
    ...
    CH --> SAME["소스는 그대로 cuda* · cublas* 이름을 쓴다"]
    HH --> SAME
```
