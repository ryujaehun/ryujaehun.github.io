...
In this engine, right after creating the cuBLAS handle, `main()` immediately does two things (`src/main.cpp:557-569`): it takes the weights out of `model.safetensors` and puts them on the GPU, and it preallocates the buffers that every computation from here on will use. This chapter reads both.
...
This chapter covers two of those steps, `loadWeights` (`src/main.cpp:79-147`) and the buffer-allocation block (`src/main.cpp:617-693`). The same initialization block also builds the RoPE frequency table (`src/main.cpp:572`) and the request queue (`src/main.cpp:583-594`), but those belong to chapters 3 and 7 respectively.
...
The function splits into four steps. First, `checkGPUStatus` (`src/main.cpp:40-62`) checks whether a device exists and, if not, prints an error and `return 1`s (`src/main.cpp:44-48`). Then it opens the file. The path is written as a literal string rather than an argument — the first sign of the assumption that the model architecture is fixed.
...
The principle behind the buffer design appeared in chapter 1: `cudaMalloc` is expensive, so grab everything at once and reuse it. The allocations all live in one block, `src/main.cpp:617-693`. Their sizes are fixed by the compile-time constants at the top of the file (`src/main.cpp:16-31`). The key one is `MAX_BUFFER_SIZE`.
...
| --- | --- | --- | --- | --- |
| `gpu_input_tokens` | `MAX_PROMPT_LEN × sizeof(int)` | 2 KiB | Temporary buffer for moving prompt token IDs to the GPU | `src/main.cpp:617-618` |
| `input_embeddings` | `MAX_PROMPT_LEN × EMBEDDING_LENGTH × 2` | 2 MiB | Token embedding output | `src/main.cpp:619-620` |
| `hidden_state` | `MAX_BUFFER_SIZE × EMBEDDING_LENGTH × 2` | 2 MiB | Layer input/output, residual accumulation | `src/main.cpp:622-623` |
| `rms_norms` | `MAX_BUFFER_SIZE × EMBEDDING_LENGTH × 2` | 2 MiB | RMSNorm output | `src/main.cpp:625-626` |
| `buf_2048_1` | `MAX_BUFFER_SIZE × EMBEDDING_LENGTH × 2` | 2 MiB | Shared between `q_proj` and `attn_scores_v` | `src/main.cpp:628-629` |
| `k_proj_temp_buf` | `MAX_PROMPT_LEN × KV_DIM × 2` | 512 KiB | Prefill K projection temporary buffer | `src/main.cpp:635-636` |
| `v_proj_temp_buf` | `MAX_PROMPT_LEN × KV_DIM × 2` | 512 KiB | Prefill V projection temporary buffer | `src/main.cpp:638-639` |
| `prefill_attn_scores` | `MAX_PROMPT_LEN² × NUM_Q_HEADS × 2` | 16 MiB | Prefill attention scores | `src/main.cpp:647-648` |
| `buf_2048_2` | `MAX_BUFFER_SIZE × EMBEDDING_LENGTH × 2` | 2 MiB | Shared between `o_proj` and `down` | `src/main.cpp:656-657` |
...
| `gpu_last_tokens` | `BATCH_SIZE × sizeof(int)` | 8 B | Tokens just before decode | `src/main.cpp:684-685` |
...
First the Q projection writes its result into `buf_2048_1`. Then the per-head attention-score computation (`src/main.cpp:301-327`) reads all of that Q. Only after that does line 343 reuse the same buffer as the scores×V output. Since the buffer is not overwritten while Q is still alive, it is safe — but that safety rests only on the static fact of the "read order" visible in the source; it is not verified by running the code.
...
At line 377 it is reused as `o_proj = buf_2048_2`, and the following cuBLAS call (`src/main.cpp:378-396`) writes the O projection output. `residualAdd` (`src/main.cpp:399`) reads that value and adds it into `hidden_state`; only then is the same buffer reused at line 470 as the down projection output. The lifetimes of the O and down results never overlap.

The aliasing continues on the decode path. `pagedAttention`'s output is again written into `buf_2048_1` (`src/main.cpp:878`) and immediately becomes the O projection's input. Why this in-place write is safe is chapter 6's subject.
