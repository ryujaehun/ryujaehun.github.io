# 1. Product Boundary and Execution Entry Point
...
its translation units (`CMakeLists.txt:49-52`). In other words, the product
...
`main` (`src/main.cpp:555-556`), weight loading `loadWeights`
(`src/main.cpp:79-147`), the prompt-streaming `prefill`
(`src/main.cpp:150-553`), and the `main` body including the decode loop
(`src/main.cpp:555-1044`). `src/kernels.cu` holds the device kernels. There are
exactly 11 `__global__` definitions, split into 7 prefill-family kernels
(`src/kernels.cu:32`, `55`, `173`, `224`, `257`, `311`, `331`) and 4
decode-family kernels (`src/kernels.cu:347`, `371`, `408`, `461`).

Headers are not counted as translation units. `CMakeLists.txt` adds `src` and
`include` as include paths (`CMakeLists.txt:62-63`). Through those paths
`src/main.cpp` pulls in three headers: `cuda_to_hip.h` (`src/main.cpp:4`),
`json.hpp` (`src/main.cpp:7`), and `kernels.cuh` (`src/main.cpp:8`). Of these,
...
(`include/json.hpp:6-7`, `include/json.hpp:68-70`), and its use in product code
is confined to a single place, safetensors header parsing: the alias declaration
(`src/main.cpp:10`), the JSON parsing (`src/main.cpp:104`), and the tensor
traversal (`src/main.cpp:106`). Although this file accounts for about 92% of the
...
| --- | --- | --- |
| `src/main.cpp` | Product translation unit. Holds the host entry point and orchestration | `CMakeLists.txt:49-52`, `src/main.cpp:555-1044` |
| `src/kernels.cu` | Product translation unit. Defines 11 `__global__` kernels | `CMakeLists.txt:49-52`, `src/kernels.cu:32-523` |
| `include/json.hpp` | Not a product translation unit. A single-header dependency brought in through the `include` path | `CMakeLists.txt:62-63`, `src/main.cpp:7` |
...
- The product binary is made from only two translation units, `src/main.cpp` and
  `src/kernels.cu` (`CMakeLists.txt:49-52`).
- `include/json.hpp` is not product code but a single-header dependency brought
  in through the include path (`CMakeLists.txt:62-63`, `src/main.cpp:7`).

## Same Source, Two Backends

`CMakeLists.txt` defines an option that defaults to off,
`option(USE_HIP "Build with HIP for AMD GPUs" OFF)` (`CMakeLists.txt:3`). This
...
- Language. If `USE_HIP`, then `project(tiny-vllm LANGUAGES CXX HIP)`
  (`CMakeLists.txt:13`); otherwise `LANGUAGES CXX CUDA` (`CMakeLists.txt:15`).
- Compiler. If not HIP, it specifies the `nvcc` path (`CMakeLists.txt:5-8`).
...
- Compile flags. It keeps separate Release/DEBUG flags per backend
  (`CMakeLists.txt:32-38`).
- Dependency packages. If HIP, `hipblas` and `hip` (`CMakeLists.txt:42-44`);
  otherwise `CUDAToolkit` (`CMakeLists.txt:46`).
- Translation unit language. If HIP, it compiles `main.cpp` and `kernels.cu` as
  HIP and defines the `USE_HIP` macro (`CMakeLists.txt:54-60`).
- Link. If HIP, `hip::host` and `roc::hipblas` (`CMakeLists.txt:65-69`);
  otherwise `CUDA::cublas` and `CUDA::cudart` (`CMakeLists.txt:71-74`).

The reason the source tolerates both backends is `src/cuda_to_hip.h`. This header
branches on a single condition,
`#if defined(USE_HIP) || defined(__HIP_PLATFORM_AMD__)` (`src/cuda_to_hip.h:6`).
On the HIP side it pulls in `hip_runtime.h`, `hip_bf16.h`, and `hipblas.h`
(`src/cuda_to_hip.h:8-10`), maps the bfloat16 type from `__nv_bfloat16` to
`__hip_bfloat16` (`src/cuda_to_hip.h:13-14`), and rewrites runtime names such as
`cudaMalloc` and `cudaMemcpy`, and BLAS names such as `cublasCreate` and
`cublasGemmEx`, into macros (`src/cuda_to_hip.h:17-31`,
`src/cuda_to_hip.h:33-46`). On the CUDA side it uses the standard headers as is
(`src/cuda_to_hip.h:52-59`). `src/main.cpp:4` and `src/kernels.cu:1` are the
points that include this header. `src/kernels.cuh` also defines the bfloat16
alias once more on its own (`src/kernels.cuh:3-9`).
...
(`src/cuda_to_hip.h:48-50`, `src/cuda_to_hip.h:58-59`). The only place that
actually consumes this constant is the five shuffle lines inside
`pagedAttentionKernel` (`src/kernels.cu:489-493`). The behavior of that kernel is
owned by Part 6.
...
standard input (`full_test.sh:1`). That sequence is 42 token IDs in the form of
the Llama-3 chat template. `test.sh` is two lines: it first calls `./build.sh`
and immediately calls `./run.sh` (`test.sh:1-2`). `build.sh` deletes and
recreates `build/` and builds with `cmake .. -G Ninja && ninja` (`build.sh:1-3`).
`run.sh` runs `./build/tiny-vllm` (`run.sh:1`).
...
The C++ entry point is `main` (`src/main.cpp:555-556`). Its first action is
creating the cuBLAS handle, and it does `return 1` on failure
(`src/main.cpp:557-563`). Immediately it loads the weights with `loadWeights`
(`src/main.cpp:566-569`), initializes the RoPE frequency table
(`src/main.cpp:572`), then sets up the paged KV cache allocator
(`src/main.cpp:574-581`). Next it prepares the request queue
(`src/main.cpp:583-594`) and the slot/batch state (`src/main.cpp:597-610`), and
allocates the compute buffers (`src/main.cpp:617-693`). The initial prefill loop
fills empty slots with prompts from the queue (`src/main.cpp:695-708`), after
which the `while (true)` decode loop runs (`src/main.cpp:720-1039`). The loop's
only break is when all active slots have disappeared while the queue is empty
(`src/main.cpp:738-746`). After the break it prints `Ok bye!`, cleans up the
handle, and does `return 0` (`src/main.cpp:1040-1044`).

There are only four host functions. They are `checkGPUStatus`
(`src/main.cpp:40-62`), `loadWeights` (`src/main.cpp:79-147`), `prefill`
(`src/main.cpp:150-553`), and `main` (`src/main.cpp:555-1044`). Since the entire
...
(`src/main.cpp:555`). However, nowhere in the body does it read `argc` or
...
vectors in chat-template form onto `queue` (`src/main.cpp:583-594`). Comments
label each prompt as "What is 2+2?" (length 17), "Name a color." (length 14),
"Say hello." (length 13), and "Capital of France?" (length 14)
(`src/main.cpp:583`, `src/main.cpp:587`, `src/main.cpp:590`,
`src/main.cpp:593`). The path too is hardcoded as a string rather than an
...
- `CMakeLists.txt:1-75` — language and backend branching, translation units,
  include paths, linking.
- `build.sh:1-3`, `run.sh:1`, `test.sh:1-2`, `full_test.sh:1` — execution path.
- `src/cuda_to_hip.h:1-61` — bfloat16, runtime, and BLAS shim and
  `WARP_FULL_MASK`.
- `src/kernels.cuh:1-28` — kernel declarations and the bfloat16 alias.
- `src/main.cpp:1-10` — the include block.
- `src/main.cpp:40-62`, `79-147`, `150-553`, `555-1044` — the four host
  functions.
- `src/main.cpp:583-594` — the hardcoded prompt queue.
- `src/main.cpp:720-746`, `1040-1044` — the decode loop's break and normal
  termination.
- `src/kernels.cu:32-523` — the 11 `__global__` kernels.
- `include/json.hpp:6-7`, `68-70` — the vendored header's SPDX license and
  version.
