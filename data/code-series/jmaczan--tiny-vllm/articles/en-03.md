# 3. The prefill path: flowing the entire prompt at once
...
Evidence that this kernel actually produces correct values is committed to the repo. `reference.txt` and `python/rms_norm_crosscheck.txt` are reference outputs generated by a Python script, and the front portions of the two files (lines 1-151) are exactly identical. That script (`python/reference.py:43-44`, `python/rms_norm.py:43-44`) defines the value of applying the first layer's `input_layernorm` to the embeddings.
...
The kernel does not compute the cos and sin values used in the rotation. `init_rope_frequencies`(`src/kernels.cu:96-152`) pre-fills `d_cos_table` and `d_sin_table` of size `[max_seq_len, head_dim]` at program startup (`src/main.cpp:572`) and uploads them to the GPU. The kernel finds its own frequency term via `token_idx * head_dim + pair_idx * 2`. Because adjacent even/odd elements were filled with the same value when the table was built, a pair uses the same c and s.
...
Now the attention scores are computed from Q and K. For each of the 32 Q heads we build one (tokens, tokens) score matrix and store it in `prefill_attn_scores`(`512×512×32`, `src/main.cpp:647-648`).
...
`k_head_idx = i / 4` is the one-line core of GQA. Q heads 0-3 use K head 0, Q heads 4-7 use K head 1, and so on: four queries share one key. The scale is `attn_alpha = 1.0f / 8.0f`(`src/main.cpp:649`), i.e. `1/sqrt(64)`, the inverse square root of `HEAD_DIM = 64`. Each call produces a (tokens, tokens) matrix `Q_head · K_head^T` scaled by `1/8` and writes it to `attn_score_head`.
...
The file's header comment (`tests/test_softmax.cu:1-52`) states that the merge kernel was compared against the earlier two-pass version on the same inputs, and every value matched to the last bfloat16 bit. That is the file's own claim. This series has no GPU, so it does not re-run that verification.
...
Then the O projection (`src/main.cpp:378-396`) maps (tokens, 2048) back to (tokens, 2048), and the residual is added. The residual is not a cuBLAS beta factor but a separate kernel.
...
Then the down projection (`src/main.cpp:471-489`) brings (tokens, 8192) back to (tokens, 2048), and `residualAdd` adds it to the hidden state. That finishes one layer, and the loop returns to the next layer's rmsNorm.

## Choosing the next token: final RMSNorm, logits, argmax

After all 16 layers, it passes through the final RMSNorm(`src/main.cpp:494`) and produces logits. The logits are a matrix product with the embedding matrix — `embed_tokens`(128256, 2048) times (tokens, 2048), giving (tokens, 128256)(`src/main.cpp:509-527`). The result is copied down to the CPU, and argmax picks the next token from the last token's row.
