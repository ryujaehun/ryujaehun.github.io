# 6. Hiding CPU scheduling behind two streams
...
`copy_done.synchronize()` (`python/minisgl/scheduler/scheduler.py:143`), this loop
...
in chapter 2 (`python/minisgl/scheduler/table.py:11`) is what is being used here.
...
chapter 4 (`python/minisgl/scheduler/cache.py:144-146`), the prefix token copy from
chapter 3 (`python/minisgl/scheduler/prefill.py:58-61`) — were all
...
(`python/minisgl/engine/engine.py:38-39`) and, on every batch, verifies it is
currently on it.
