# 7. 장부가 커널 인자로 바뀌는 지점과 CUDA graph
...
다(`python/minisgl/attention/fa.py:76`). 6편에서 본 `wait_stream` 이 필요한 이유가
여기에도 있다.
...
있다(`python/minisgl/attention/base.py:24-34`). 즉 "커널을 부르는 법"뿐 아니라
...
(`python/minisgl/engine/engine.py:98`)이 여기서 만난다. 모자란 자리는 더미 요청으로
