...
때문이다(`python/minisgl/server/launch.py:24-25`). 이 루프가 끝나야 HTTP 서버가
...
(`python/minisgl/server/api_server.py:116-123`). 스트리밍 응답은 그 이벤트를
기다리며 쌓인 조각을 흘려보낸다.
...
(`python/minisgl/message/tokenizer.py:27-31`), 디토크나이저는 그것을 모아
...
(`python/minisgl/scheduler/io.py:116-121`). 개수를 먼저 맞추지 않으면 어떤 rank 는
...
복원할 때는 그 이름으로 클래스를 찾는다(`python/minisgl/message/utils.py:52-68`).
