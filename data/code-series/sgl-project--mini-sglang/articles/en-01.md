...
— `python/minisgl/server/launch.py:54-69`
...
— `python/minisgl/server/launch.py:71-103`
...
— `python/minisgl/server/launch.py:105-111`

However many schedulers start, exactly one ack arrives from them, because only
the primary rank sends one (`python/minisgl/server/launch.py:24-25`). The HTTP
...
— `python/minisgl/server/api_server.py:265-278`
...
— `python/minisgl/server/api_server.py:109-114`
...
(`python/minisgl/server/api_server.py:116-123`), and the streaming response waits
...
— `python/minisgl/tokenizer/server.py:87-101`
...
— `python/minisgl/message/backend.py:32-36`
...
(`python/minisgl/message/tokenizer.py:27-31`), and the detokenizer gathers those
into `UserReply` objects for the frontend
(`python/minisgl/tokenizer/server.py:71-85`).
...
— `python/minisgl/scheduler/io.py:96-107`
...
exactly that many (`python/minisgl/scheduler/io.py:116-121`). Without agreeing on
...
(`python/minisgl/message/utils.py:52-68`). No schema file, no code generation.
...
— `tests/misc/test_serialize.py:32-33`
