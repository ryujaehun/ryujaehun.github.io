...
— `python/minisgl/attention/fa.py:67-75`
...
— `python/minisgl/attention/fa.py:79-82`

`CPU_KWARGS` carries `pin_memory: True` and every transfer is `non_blocking=True`
(`python/minisgl/attention/fa.py:76`). The `wait_stream` from chapter 6 is needed
here too.
...
— `python/minisgl/attention/fa.py:84-89`
...
— `python/minisgl/attention/base.py:18-20`

Besides `forward`, both `prepare_metadata` and `prepare_for_replay` are abstract
(`python/minisgl/attention/base.py:24-34`). So a backend owns not just "how to call
...
— `python/minisgl/attention/__init__.py:22`
...
— `python/minisgl/attention/__init__.py:57-64`
...
— `python/minisgl/layers/attention.py:47-56`
...
— `python/minisgl/engine/graph.py:160-166`
...
(`python/minisgl/engine/engine.py:98`), meet here. The shortfall is filled with
...
— `python/minisgl/engine/graph.py:149-150`
...
— `python/minisgl/engine/graph.py:152-157`
