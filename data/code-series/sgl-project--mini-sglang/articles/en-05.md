# 5. The radix cache that reuses prefixes, and eviction
...
It calls a prebuilt C++ module (`python/minisgl/kernel/radix.py:13-15`). Prefix
...
(`python/minisgl/kvcache/radix_cache.py:228-229`), so the least recently used leaf
...
The comment block above it (`python/minisgl/scheduler/cache.py:56-66`) lays the
...
(`python/minisgl/kvcache/naive_cache.py:12-13`). Not one line of scheduler code
...
(`tests/core/test_cache_allocate.py:46-54`) — a check made possible by the fact,
