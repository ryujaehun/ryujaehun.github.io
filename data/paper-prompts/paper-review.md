...
렌더링은 KaTeX 가 한다. 아래를 어기면 그 수식이 붉은 오류로 나오거나
페이지가 깨진다. 실측으로 잡힌 것들이다.

- **수식 안에서 마크다운 이스케이프를 쓰지 마라.** `$\tau\_g$` 가 아니라
  `$\tau_g$` 다. `\_`, `\*`, `\<`, `\>`, `\$` 를 수식 안에 넣지 마라.
- **수식 구분자를 이스케이프하지 마라.** `\$...\$` 는 수식으로 인식되지
  않는다. `$...$` 로 써라.
- **수식 안에 마크다운 강조를 쓰지 마라.** `$**39.6**_{2.1}$` 는 깨진다.
  강조가 필요하면 수식 밖에서 `**$39.6_{2.1}$**` 로 해라.
- **아래첨자·위첨자 뒤를 비우지 마라.** `$95.0_$` 는 오류다. 값이 없으면
  첨자를 쓰지 마라: `$95.0$`.
- **없는 명령을 만들지 마라.** `\na` 같은 것은 없다. "해당 없음" 은
  `$9_{\text{n/a}}$` 처럼 `\text{}` 안에 넣어라.
- **산문의 달러 기호는 인라인 코드로 감싸라.** 비용을 적을 때 `$/1M
  tokens` 로 쓰면 KaTeX 가 그 뒤를 수식으로 삼킨다. `` `$/1M tokens` ``
  처럼 백틱으로 감싸라. **이스케이프(`\$`)는 이 블로그에서 듣지 않는다** —
  Goldmark 가 `\$` 를 `$` 로 되돌리고, 그 `$` 를 다시 KaTeX 가 집는다
  (실측). 수식 안이라면 `$\$3.06$` 처럼 수식으로 쓰는 것도 된다.
- **문장 속 수식은 `$` 하나로 감싸라.** `$$t$$` 처럼 `$$` 를 문장 안에
  쓰면 안 된다. `$$` 는 블록 구분자라 Goldmark 가 그 자리에서 단락을 끊고
  수식을 단락 밖으로 빼낸다. 그러면 KaTeX 가 그 텍스트를 잡지 못해 `$$t$$`
  가 본문에 그대로 보이고, 앞뒤로 빈 줄이 생겨 한 문장이 여러 조각으로
  쪼개진다 (실측: 한 편에서 132회, 문단이 통째로 깨졌다). 기호 하나를
  가리킬 때도 `$t$` 다.
- **블록 수식은 제 줄에 홀로 둬라.** `$$` 여는 줄과 닫는 줄 사이에 수식만
  두고, 앞뒤는 빈 줄로 띄운다. 산문과 같은 줄에 두지 마라.
- `\text{}` 안에는 한국어를 넣어도 된다. 그 밖의 수식 기호는 손대지 마라.
